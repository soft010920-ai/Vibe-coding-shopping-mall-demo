...
export default function OrderPage({ 
  ...
  const createOrder = async (transactionId = '') => {
    ...
    try {
      ...
      // 네트워크 오류 등 예외 상황에서도 실패 페이지로 이동
      if (onNavigateToOrderFailed) {
        onNavigateToOrderFailed(`주문 생성 중 오류가 발생했습니다: ${error.message}`)
      } else {
  ...
  const requestPayment = () => {
    ...
    IMP.request_pay({
      ...
      if (rsp.success) {
        ...
        const errorMsg = `결제에 실패했습니다.\n에러 메시지: ${rsp.error_msg || '알 수 없는 오류'}`
        
        // 주문 실패 페이지로 이동
        if (onNavigateToOrderFailed) {
          onNavigateToOrderFailed(errorMsg)
        } else {
          alert(errorMsg)
          setSubmitting(false)
        }
